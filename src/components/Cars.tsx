...
export default function Cars() {
  const handleWhatsAppClick = (carName: string) => {
    ...
    const whatsappUrl = `https://wa.me/212669458458?text=${encodeURIComponent(message)}`;
          ...
          {cars.map((car) => (
            <div key={car.id} className="bg-white rounded-lg shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300">
              <div className="relative h-48">
                ...
                />
                <div className="absolute top-0 right-0 bg-[#A7984E] text-white px-4 py-2 rounded-bl-lg">
                  {car.price}
                </div>
              </div>
              ...
              <div className="p-6">
                ...
                <ul className="mb-6 space-y-2">
                  {car.features.map((feature, index) => (
                    <li key={index} className="text-gray-600 flex items-center">
                      <span className="w-2 h-2 bg-[#A7984E] rounded-full mr-2 flex-shrink-0"></span>
                ...
                <button
                  ...
                  className="w-full bg-[#A7984E] text-white px-4 py-2 rounded-lg flex items-center justify-center hover:bg-[#432F25] transition-colors"
                >
